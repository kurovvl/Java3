...
import ru.geekbrains.july_chat.chat_server.DBHandler;
...
//    }
    ...
    public static String codePass(String password) {
        String result = "";
        try {
    ...
    private void initBaseRecords() {
        try {
            if (super.selectQuery("select * from " + tableName).next()) {
                return;
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        createUser("log1", "pass", "nick1");
        createUser("log2", "pass", "nick2");
        createUser("log3", "pass", "nick3");
        createUser("log4", "pass", "nick4");
    }
